...
const StarIcon: React.FC<{
  ...
  className?: string
}> = ({ variant, size, color, style, className = '' }) => {
  ...
  for (let i = 0; i < filledStars; i++)
    stars.push(
      <button
        ...
        aria-label={isInteractive ? `Rate ${i + 1} out of ${max} stars` : ''}
      >
    ...
    stars.push(
      <div key="half" className="relative">
        <button
          ...
          className={isInteractive ? 'cursor-pointer' : ''}
          aria-label={
            isInteractive ? `Rate ${filledStars + 0.5} out of ${max} stars` : ''
          }
        >
  ...
  for (let i = 0; i < emptyStars; i++)
    stars.push(
      <button
        key={`empty-${i}`}
        onClick={() =>
          handleStarClick(filledStars + (hasHalfStar ? 1 : 0) + i + 1)
        }
        disabled={!isInteractive}
        className={isInteractive ? 'cursor-pointer' : ''}
        aria-label={
