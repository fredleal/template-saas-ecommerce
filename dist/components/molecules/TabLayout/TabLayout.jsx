...
export const TabLayout = ({ tabs, variant = 'primary', orientation = 'horizontal', initialIndex = 0, onTabChange, className = '', ...props }) => {
  ...
  `;
    ...
    return (<div className={containerClasses.trim()} data-testid="tab-layout" {...props}>
      {/* Tab List */}
      <div role="tablist" aria-label="Tab navigation" className={`
        ...
        {tabs.map((tab, index) => {
            ...
            return (<Button key={tab.label} id={`tab-${index}`} role="tab" variant={isActive ? 'primary' : buttonVariant} size="sm" onClick={() => handleTabClick(index)} aria-selected={isActive} aria-controls={`tabpanel-${index}`} tabIndex={isActive ? 0 : -1} className={`
