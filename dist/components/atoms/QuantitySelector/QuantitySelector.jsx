...
export const QuantitySelector = ({ value, min = 1, max = 99, onChange, disabled = false, size = 'md', className = '', }) => {
    ...
    const baseClasses = 'inline-flex items-center border border-gray-300 rounded-md';
    ...
    return (<div className={containerClasses} data-testid="quantity-selector">
      <button type="button" onClick={handleDecrement} disabled={isDecrementDisabled} className={buttonClasses} aria-label="Decrease quantity" data-testid="quantity-decrement">
        −
      </button>
      ...
      <button type="button" onClick={handleIncrement} disabled={isIncrementDisabled} className={buttonClasses} aria-label="Increase quantity" data-testid="quantity-increment">
        +
      </button>
    </div>);
};
//# sourceMappingURL=QuantitySelector.jsx.map