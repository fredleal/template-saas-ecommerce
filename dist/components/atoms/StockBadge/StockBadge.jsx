...
export const StockBadge = ({ stock, variant, size = 'md', className = '', }) => {
    ...
    return (<span role="status" aria-label={`${variantText[currentVariant]}: ${stock} items`} className={classes}>
      <span className={`h-2 w-2 rounded-full ${currentVariant === 'inStock' ? 'bg-green-500' : currentVariant === 'lowStock' ? 'bg-yellow-500' : 'bg-red-500'}`}/>
      <span>{variantText[currentVariant]}</span>
      {stock > 0 && <span className="font-semibold">({stock})</span>}
    </span>);
};
//# sourceMappingURL=StockBadge.jsx.map